...
def _cluster_utilization(custom_api: Any, nodes: list[Any]) -> tuple[float, float] | None:
    """Return cluster CPU and memory utilization using node metrics and allocatable capacity."""
    if not nodes:
        return None
    ...
    try:
        ...
        for item in items:
            ...
            cpu_used = _cpu_to_cores(str(usage.get("cpu", "0")))
            memory_used = _memory_to_bytes(str(usage.get("memory", "0")))
...
def get_telemetry() -> dict[str, Any]:
    """Return live pod counts and best-available resource utilization."""
    try:
